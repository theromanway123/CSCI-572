...
if ($query) {
	...
	require_once('/home/rom/solr-php-client/Apache/Solr/Service.php');
...
<body>
	<form accept-charset="utf-8" method="get" class="">
		<center>
	...
	?>
		...
		$csv = array_map('str_getcsv', file('/home/rom/URLtoHTML_latimes_news.csv'));
		foreach ($results->response->docs as $doc) {
			...
			if ($url == "" || $url == null) {
				foreach ($csv as $row) {
					$cmp = "/home/rom/solr-7.7.3/data/" . $row[0];
					if ($id == $cmp) {
						$url = $row[1];
						unset($row);
						break;
